        ...
        {tasks.map((task) => (
          <div key={task.id} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {task.subject} • {typeLabel[task.type]}
                </p>
              </div>
              <span className={`text-xs px-2 py-1 rounded-full ${statusBadge[task.status]}`}>{statusLabel[task.status]}</span>
            </div>
            <div className="mt-2 flex flex-wrap gap-2 text-xs text-slate-600">
              <span className={`px-2 py-1 rounded-full ${priorityBadge[task.priority]}`}>{priorityLabel[task.priority]}</span>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <button onClick={() => onEdit(task)} className="text-xs px-2 py-1 rounded border border-slate-200 hover:bg-slate-100">
                Éditer
              </button>
              <button onClick={() => onDelete(task)} className="text-xs px-2 py-1 rounded border border-rose-200 text-rose-600 hover:bg-rose-50">
                Supprimer
              </button>
            </div>
          </div>
